...
def triangleareabycoordinates(x1, y1, x2, y2, x3, y3):
	# your code goes here
	if(type(x1)==int or float and type(y1)==int or float and type(x2)==int or float and type(y2)==int or float and type(x3)==int or float and type(y3)==int or float ):
		a = abs((1/2)*((x1*(y2-y3)+x2*(y3-y1)+x3*(y1-y2))))
		return float(a)
	# pass